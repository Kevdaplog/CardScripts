...
card* duel::new_card(uint32_t code) {
	...
	if (code != TEMP_CARD_ID) {
		...
		// Se questa carta ha un alias, copia solo il nome (code) dalla carta originale
		// e non gli altri stat (ATK, DEF, level, type, ecc.)
		if (pcard->data.alias != 0) {
			uint32_t original_alias = pcard->data.alias;
			// Azzera tutti gli stat tranne code e alias
			pcard->data.clear();
			pcard->data.alias = original_alias;
		}
	}
